...
- 当`Zset`的所有数据小于64字节时，使用`ziplist`进行存储， 否则使用`skiplist`(跳表)
...
-  `RDB`适合做备份、灾难恢复，因位`RDB`是一个非常紧凑的文件，它保持了`Redis`在某个时间点的数据集；
  - 比如说，你可以在最近的24小时内，每小时备份一次`RDB`文件，并且在每个月的每一天，也备份一个`RDB`文件
  - 这样的话，即使遇上问题，也可以随时将数据集还原到不同的版本
- `RDB`可以最大化`Redis`的性能；
  - 父进程在保存`RDB`文件时，唯一要做的就是`fork`出一个子进程；
  - 然后这个子进程就会处理接下来的所有保存工作，父进程无需执行任何磁盘`IO`操作
- `RDB` 在恢复大数据集的速度要比`AOF`的恢复快

##### `RDB`方案的缺点

- `RDB`数据安全性不如`AOF`，因为`RDB`方式没办法做到实时持久化；
  - 如果服务器宕机的话，采用`RDB`的方式会造成某个时间段内数据的丢失；
  - 比如我们设置10分钟同步一次或者5分钟到达1000次写入就同步一次，那么如果还没达到触发条件，服务器死机了，这个时间段的数据会丢失
- `RDB`持久化过程中对服务性能影响大；
  - 使用`bgsave`命令在`fork`子进程时，如果数据量太大，`fork`的过程也会发生阻塞，另外，`fork`子进程会消耗内存

###### `AOF`方案的优点

- `AOF`数据完全性更高，写入性能更好；
  - 因为`AOF`有不同的同步策略：完全不同步、每秒同步、每次操作同步，默认是每秒同步，即使服务宕机也仅会损失一秒钟的写入时间；
  - 同时`AOF`增加日志的方式是追加操作，即使在写日志的过程中服务器宕机，也可以通过`redis-check-aof`工具修复
- 如果`AOF`文件过大，`Redis`会在后台自动重写`AOF`文件。重写后会是使`AOF`文件压缩到最小所需的指令集
- `AOF`适合修复误操作：
  - `AOF`文件是有序保存数据的所有写入操作，容易理解和分析；
  - 即使不小心误操作删除数据，也可以通过分析`AOF`文件进行恢复
  
#### `AOF`方案的缺点

- 相同数据量下，`AOF`文件通常体积比`RDB`大；
  - 因为`AOF`是存指令的，而`RDB`是所有指令的结果快照
- 在大量写入和载入的时候，`AOF`的效率会比`RDB`低
  - 因为大量写入，`AOF`会执行更多的保存命令；
